...
class KoverDataset(object):
    ...
    def get_split(self, name):
        ...
        return KoverDatasetSplit(
            name,
            split.attrs["train_proportion"],
            # Backwards compatibility with datasets without test_proportion
            split.attrs["test_proportion"]
            if "test_proportion" in split.attrs
            else 1.0 - split.attrs["train_proportion"],
...
class KoverDatasetFold(object):
    def __init__(
        self, name, train_genome_idx, test_genome_idx, unique_risks, unique_risk_by_kmer, unique_risk_by_anti_kmer
    ):
        self.name = name
        self.train_genome_idx = train_genome_idx
        self.test_genome_idx = test_genome_idx
