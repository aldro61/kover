...
def split_with_ids(
    ...
    def _parse_ids(ids_file, learning_step):
        ...
        if len(ids_not_in_ds) > 0:
            error_callback(
                Exception(
                    "The %s genome identifiers contain IDs that are not in the dataset: %s"
    ...
    train_ids = _parse_ids(ids_file=train_ids_file, learning_step="training")

    test_ids = _parse_ids(ids_file=test_ids_file, learning_step="testing")
...
def _split(
    ...
    kmer_risks = (len(train_pos_idx) - kmer_matrix.sum_rows(train_pos_idx)[: dataset.kmer_count]).astype(
        np.float
    )  # n positive errors
    ...
    unique_risks, unique_risk_by_kmer_and_antikmer = np.unique(
        np.hstack((kmer_risks, anti_kmer_risks)), return_inverse=True
    )
    del kmer_risks, anti_kmer_risks
    split.create_dataset("unique_risks", data=unique_risks)
    split.create_dataset(
        "unique_risk_by_kmer",
        data=unique_risk_by_kmer_and_antikmer[: dataset.kmer_count],
        dtype=_minimum_uint_size(len(unique_risks)),
    )
    split.create_dataset(
    ...
    if n_folds > 0:
        ...
        for fold in range(n_folds):
            ...
            # XXX: In the following 2 lines, we trim the result of sum_rows to keep only the presence rule classifications.
            kmer_risks = (
                len(fold_train_pos_idx) - kmer_matrix.sum_rows(fold_train_pos_idx)[: dataset.kmer_count]
            ).astype(
                np.float
            )  # n positive errors
...
def _validate_split(dataset, split_name, train_idx, test_idx, n_folds, warning_callback, error_callback):
    ...
    if n_folds > len(train_idx):
        error_callback(
            Exception(
                "There cannot be more cross-validation folds (%d) than genomes in the training set"
