...
struct Options_192
{
	Options_192 (Storage* storage, size_t kmerSize,const unordered_map<bitset<192>, unsigned long>& k_map, KmerLister192::packing_type* p_buffer)
	:storage(storage), kmerSize(kmerSize), k_map(k_map), p_buffer(p_buffer) {}
	Storage*   storage;
	size_t     kmerSize;
	const unordered_map<bitset<192>, unsigned long>& k_map;
	KmerLister192::packing_type* p_buffer;
};	

template<size_t span> struct Functor_Analyse_All_192 { void operator () (Options_192 options)
{
	Storage*   storage  = options.storage;
	size_t     kmerSize = options.kmerSize;
	const unordered_map<bitset<192>, unsigned long>& k_map = options.k_map;
	KmerLister192::packing_type* p_buffer = options.p_buffer;
	...
	for (itKmers->first(); !itKmers->isDone(); itKmers->next())
	{
		 ...
		 * ################################
		 * */
		bitset<192> bits (count.value.getVal());
		for(int j = 1; j < 3; j++)
		{
			unsigned int limit;
			j == 2 ? limit = (2*kmerSize-128) : limit = 64;
			bitset<64> temp ((count.value>>(j *64)).getVal());
	...
	template<size_t span> struct Functor_Analyse_Filter_192 { void operator () (Options_192 options)
{
	Storage*   storage  = options.storage;
	size_t     kmerSize = options.kmerSize;
	const unordered_map<bitset<192>, unsigned long>& k_map = options.k_map;
...
struct Parameter_192
{
	Parameter_192 (Storage* storage, size_t kmerSize, unordered_map<bitset<192>, unsigned long>& k_map, unsigned long* p_nb_kmers, unordered_set<bitset<192>>& k_set)
...
template<size_t span> struct Functor_Read_All_192  {  void operator ()  (Parameter_192 parameter)
{
	Storage*   storage  = parameter.storage;
	size_t     kmerSize = parameter.kmerSize;
	unordered_map<bitset<192>, unsigned long>& k_map = parameter.k_map;
...
template<size_t span> struct Functor_Read_Filter_192  {  void operator ()  (Parameter_192 parameter)
{
	Storage*   storage  = parameter.storage;
	size_t     kmerSize = parameter.kmerSize;
	unordered_map<bitset<192>, unsigned long>& k_map = parameter.k_map;
	unsigned long* p_nb_kmers = parameter.p_nb_kmers;
	unordered_set<bitset<192>>& k_set = parameter.k_set;
...
/** */
KmerLister192::KmerLister192 (size_t kmerSize):kmerSize(kmerSize)
{}
/** */
void KmerLister192::analyse (string input_file, string output_file, string filter)
{
	string line;
	...
	try
	{
		if (h5_list)
		{
			while (getline(h5_list, line))
			{
				...
				if (filter == "nothing")
				{
					Integer::apply<Functor_Read_All_192,Parameter_192> (kmerSize, Parameter_192(storage, kmerSize, k_map, &nb_kmers, k_set));
				}
				else if (filter == "singleton")
				{
					Integer::apply<Functor_Read_Filter_192,Parameter_192> (kmerSize, Parameter_192(storage, kmerSize, k_map, &nb_kmers, k_set));	
				}
			}
		}
	}
	...
	//h5_file = H5Fopen((getInput()->getStr(STR_URI_OUTPUT)).c_str(), H5F_ACC_RDWR, H5P_DEFAULT);

	// Creating dataspace
	hsize_t dim[2] = {(unsigned long long) (ceil((1.0 * nb_genomes) / KMER_MATRIX_PACKING_SIZE_192)), nb_kmers};
	...
	// Defining datatype
	if (KMER_MATRIX_PACKING_SIZE_192 == 32)
	{
		datatype = H5Tcopy(H5T_NATIVE_UINT);
	}	
	else if (KMER_MATRIX_PACKING_SIZE_192 == 64)
	{
	...
	dataset = H5Dcreate2(h5_file, "kmer_matrix", datatype, dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	
	// Creating files name buffer
	string buffer_genomes[KMER_MATRIX_PACKING_SIZE_192];
	...
	try
	{
		if (dsk_list)
		{
			...
			for (unsigned int i = 0; i < ceil((1.0 * nb_genomes) / KMER_MATRIX_PACKING_SIZE_192); i++)
			{
				...
				while (files_checked < KMER_MATRIX_PACKING_SIZE_192 && en_cours)
				{
				...
				for (int genome = (files_checked - 1); genome >= 0; genome--)
				{
					...
					if (filter == "nothing")
					{
						Integer::apply<Functor_Analyse_All_192,Options_192> (kmerSize, Options_192(storage, kmerSize, k_map, buffer));
					}
					else if (filter == "singleton")
					{
						Integer::apply<Functor_Analyse_Filter_192,Options_192> (kmerSize, Options_192(storage, kmerSize, k_map, buffer));
					}
		...
		dataset_kmers = H5Dcreate2(h5_file, "kmer_sequences", datatype_kmers, dataspace_kmers, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
		dataset_index = H5Dcreate2(h5_file, "kmer_by_matrix_column", datatype_index, dataspace_index, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
		
		// Iterating over the unordered map
		unordered_map<bitset<192>, unsigned long>::const_iterator iter;
		iter = k_map.begin();
...
void KmerLister192::bit_shift(packing_type* p_buffer, unsigned long* p_nb_kmers)
{
...
string KmerLister192::convert(bitset<192> bits)
{
	char seq[kmerSize + 1];
