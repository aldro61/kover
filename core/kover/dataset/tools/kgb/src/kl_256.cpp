...
struct Options_256
{
	Options_256 (Storage* storage, size_t kmerSize,const unordered_map<bitset<256>, unsigned long>& k_map, KmerLister256::packing_type* p_buffer)
	:storage(storage), kmerSize(kmerSize), k_map(k_map), p_buffer(p_buffer) {}
	Storage*   storage;
	size_t     kmerSize;
	const unordered_map<bitset<256>, unsigned long>& k_map;
	KmerLister256::packing_type* p_buffer;
};	

template<size_t span> struct Functor_Analyse_All_256 { void operator () (Options_256 options)
{
	Storage*   storage  = options.storage;
	size_t     kmerSize = options.kmerSize;
	const unordered_map<bitset<256>, unsigned long>& k_map = options.k_map;
	KmerLister256::packing_type* p_buffer = options.p_buffer;
	...
	for (itKmers->first(); !itKmers->isDone(); itKmers->next())
	{
		 ...
		 * ################################
		 * */
		bitset<256> bits (count.value.getVal());
		for(int j = 1; j < 4; j++)
		{
			unsigned int limit;
			j == 3 ? limit = (2*kmerSize-192) : limit = 64;
	...
	template<size_t span> struct Functor_Analyse_Filter_256 { void operator () (Options_256 options)
{
	Storage*   storage  = options.storage;
	size_t     kmerSize = options.kmerSize;
	const unordered_map<bitset<256>, unsigned long>& k_map = options.k_map;
...
struct Parameter_256
{
	Parameter_256 (Storage* storage, size_t kmerSize, unordered_map<bitset<256>, unsigned long>& k_map, unsigned long* p_nb_kmers, unordered_set<bitset<256>>& k_set)
...
template<size_t span> struct Functor_Read_All_256  {  void operator ()  (Parameter_256 parameter)
{
	Storage*   storage  = parameter.storage;
	size_t     kmerSize = parameter.kmerSize;
	unordered_map<bitset<256>, unsigned long>& k_map = parameter.k_map;
...
template<size_t span> struct Functor_Read_Filter_256  {  void operator ()  (Parameter_256 parameter)
{
	Storage*   storage  = parameter.storage;
	size_t     kmerSize = parameter.kmerSize;
	unordered_map<bitset<256>, unsigned long>& k_map = parameter.k_map;
	unsigned long* p_nb_kmers = parameter.p_nb_kmers;
	unordered_set<bitset<256>>& k_set = parameter.k_set;
...
{}
/** */
void KmerLister256::analyse (string input_file, string output_file, string filter)
{
	string line;
	...
	try
	{
		if (h5_list)
		{
			while (getline(h5_list, line))
			{
				...
				if (filter == "nothing")
				{
					Integer::apply<Functor_Read_All_256,Parameter_256> (kmerSize, Parameter_256(storage, kmerSize, k_map, &nb_kmers, k_set));
				}
	...
	// Creating dataspace
	hsize_t dim[2] = {(unsigned long long) (ceil((1.0 * nb_genomes) / KMER_MATRIX_PACKING_SIZE_256)), nb_kmers};
	...
	// Defining datatype
	if (KMER_MATRIX_PACKING_SIZE_256 == 32)
	{
		datatype = H5Tcopy(H5T_NATIVE_UINT);
	}	
	else if (KMER_MATRIX_PACKING_SIZE_256 == 64)
	{
	...
	string buffer_genomes[KMER_MATRIX_PACKING_SIZE_256];
	...
	try
	{
		if (dsk_list)
		{
			...
			for (unsigned int i = 0; i < ceil((1.0 * nb_genomes) / KMER_MATRIX_PACKING_SIZE_256); i++)
			{
				...
				while (files_checked < KMER_MATRIX_PACKING_SIZE_256 && en_cours)
				{
				...
				for (int genome = (files_checked - 1); genome >= 0; genome--)
				{
					...
					if (filter == "nothing")
					{
						Integer::apply<Functor_Analyse_All_256,Options_256> (kmerSize, Options_256(storage, kmerSize, k_map, buffer));
					}
					else if (filter == "singleton")
					{
		...
		unordered_map<bitset<256>, unsigned long>::const_iterator iter;
		iter = k_map.begin();
...
void KmerLister256::bit_shift(packing_type* p_buffer, unsigned long* p_nb_kmers)
{
...
string KmerLister256::convert(bitset<256> bits)
{
	char seq[kmerSize + 1];
