...
template<size_t span> struct Functor_Analyse_All_256 { 
    void operator () (Options_256 options)
    {
...
template<size_t span> struct Functor_Analyse_Filter_256 { 
    void operator () (Options_256 options)
    {
        Storage*   storage  = options.storage;
        size_t     kmerSize = options.kmerSize;
...
template<size_t span> struct Functor_Read_All_256  {  
    void operator ()  (Parameter_256 parameter)
    {
...
template<size_t span> struct Functor_Read_Filter_256  {  
    void operator ()  (Parameter_256 parameter)
    {
        Storage*   storage  = parameter.storage;
        size_t     kmerSize = parameter.kmerSize;
...
{}
/** */
void KmerLister256::analyse (string input_file, string output_file, string filter, unsigned int compression, unsigned int chunk_size, Callable& progressbar)
{
	string line;
			...
			for (unsigned int i = 0; i < ceil((1.0 * nb_genomes) / KMER_MATRIX_PACKING_SIZE_256); i++)
			{
				...
				for (unsigned int genome = 0; genome < files_checked; genome++)
				{
					...
					else
					{
						...
						unsigned int empty_rows = KMER_MATRIX_PACKING_SIZE_256 - files_checked;
						if (empty_rows)
						{
