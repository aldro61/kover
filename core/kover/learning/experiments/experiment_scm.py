...
def _cv_score_hp(hp_values, max_rules, dataset_file, split_name, rule_blacklist):
    ...
    for i, fold in enumerate(folds):
        ...
        test_predictions_by_model_length = np.array(
            _duplicate_last_element(test_predictions_by_model_length, max_rules)
        )
        fold_score_by_model_length[i] = _get_binary_metrics(
            predictions=test_predictions_by_model_length, answers=dataset.phenotype.metadata[test_example_idx]
        )["risk"]
...
def _full_train(
    dataset, split_name, model_type, p, max_rules, max_equiv_rules, rule_blacklist, random_generator, progress_callback
):
    ...
    def _iteration_callback(iteration_infos, model_type, equivalent_rules):
        ...
        if len(iteration_infos["equivalent_rules_idx"]) > max_equiv_rules:
            logging.debug(
                "There are more equivalent rules than the allowed maximum. Subsampling %d rules." % max_equiv_rules
            )
            random_idx = random_generator.choice(
                len(iteration_infos["equivalent_rules_idx"]), max_equiv_rules, replace=False
            )
            random_idx.sort()
    ...
    predictor.fit(
        ...
        tiebreaker=partial(
            _tiebreaker,
            rule_risks=np.hstack((split.unique_risk_by_kmer[...], split.unique_risk_by_anti_kmer[...])),
            model_type=model_type,
        ),
        iteration_callback=partial(_iteration_callback, model_type=model_type, equivalent_rules=model_equivalent_rules),
    )
...
def _bound(train_predictions, train_answers, train_example_idx, model, delta, max_genome_size, rule_classifications):
    ...
    return 1.0 - exp(
        (-1.0 / (m - mz - r))
        * (
            ln(comb(m, mz, exact=True))
            + ln(comb(m - mz, r, exact=True))
            + h_card * ln(2 * Z_card)
            + ln(pi ** 6 * (h_card + 1) ** 2 * (r + 1) ** 2 * (mz + 1) ** 2 / (216 * delta))
        )
    )
...
def _bound_selection(
    ...
    for hp, score, model, rule_importances, equiv_rules in pool.imap_unordered(
        ...
        if (
            ...
            or (score == best_hp_score and hp[2] == best_hp["max_rules"] and abs(1.0 - hp[1]) < abs(1.0 - best_hp["p"]))
        ):
            best_hp["model_type"] = hp[0]
            best_hp["p"] = hp[1]
            best_hp["max_rules"] = hp[2]
...
def learn_SCM(
    ...
    if parameter_selection == "bound":
        if bound_delta is None or bound_max_genome_size is None:
            error_callback(
                Exception("Bound selection cannot be performed without delta and the maximum genome length.")
            )
        ...
        best_hp_score, best_hp, best_model, best_rule_importances, best_predictor_equiv_rules = _bound_selection(
            dataset_file=dataset_file,
            split_name=split_name,
    ...
    classifications["train_correct"] = (
        dataset.genome_identifiers[train_example_idx[train_predictions == train_answers].tolist()].tolist()
