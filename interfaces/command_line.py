...
class KoverDatasetCreationTool(object):
    ...
    def from_tsv(self):
        parser = argparse.ArgumentParser(
            prog="kover dataset create from-tsv",
            description="Creates a Kover dataset from genomic data and optionally " "phenotypic metadata",
        )
        parser.add_argument("--genomic-data", help="A tab-separated file containing the k-mer matrix.", required=True)
        parser.add_argument(
            "--phenotype-description", help="An informative description that is assigned to the" " phenotypic metadata."
        )
        ...
        if args.verbose:
            logging.basicConfig(
                level=logging.DEBUG, format="%(asctime)s.%(msecs)d %(levelname)s %(module)s - %(funcName)s: %(message)s"
            )
            ...
            def progress(task_name, p):
                if task_name != progress_vars["current_task"]:
                    ...
                    progress_vars["pbar"] = ProgressBar(
                        widgets=["%s: " % task_name, Percentage(), Bar(), Timer()], maxval=1.0
                    )
    ...
    def from_contigs(self):
        parser = argparse.ArgumentParser(
            prog="kover dataset create from-contigs",
            description="Creates a Kover dataset from genomic data and optionally " "phenotypic metadata",
        )
        parser.add_argument(
            "--genomic-data",
            help="A tab-separated file with one line per genome in the format "
        ...
        parser.add_argument(
            "--singleton-kmers",
            help="Include k-mers that only occur in one genome. Disabled by " "default.",
            default=False,
            action="store_true",
        )
        parser.add_argument(
            "--n-cpu",
            "--n-cores",
            help="The number of cores used by DSK. The default value is 0 (all cores).",
            default=0,
        )
        parser.add_argument(
            "--compression",
        ...
        parser.add_argument(
            "--temp-dir",
            help="Output directory for temporary files. The default is the system's temp dir.",
            default=gettempdir(),
        )
        parser.add_argument("-x", "--progress", help="Shows a progress bar for the execution.", action="store_true")
        parser.add_argument("-v", "--verbose", help="Sets the verbosity level.", default=False, action="store_true")
        ...
        from_contigs(
            contig_list_path=args.genomic_data,
            output_path=args.output,
    ...
    def from_reads(self):
        parser = argparse.ArgumentParser(
            prog="kover dataset create from-reads",
            description="Creates a Kover dataset from genomic data and optionally " "phenotypic metadata",
        )
        parser.add_argument(
            "--genomic-data",
        ...
        parser.add_argument(
            "--kmer-min-abundance",
            help="The minimum number of times a k-mer must be found in a read file "
        ...
        from_reads(
            reads_folders_list_path=args.genomic_data,
            output_path=args.output,
            kmer_size=args.kmer_size,
...
class KoverDatasetTool(object):
    ...
    def create(self):
        ...
        parser = argparse.ArgumentParser(
            usage="""%(prog)s dataset create <data source> [<args>]
The two available data sources are:
    ...
    from-reads      Create a Kover dataset from reads"""
        ...
        parser.add_argument(
            "datasource", help="The type of genomic data to be used.", choices=creation_tool.available_data_sources
        )
    ...
    def info(self):
        parser = argparse.ArgumentParser(
            prog="kover dataset info", description="Prints information about the content of a dataset"
        )
        ...
        parser.add_argument(
            "--genome-source",
            help="Prints the source (e.g.: path) from which the genomic data was" " acquired.",
            action="store_true",
        )
        parser.add_argument(
            "--genome-ids", help="Prints the identifiers of the genomes in the dataset.", action="store_true"
        )
        parser.add_argument("--genome-count", help="Prints the number of genomes in the dataset.", action="store_true")
        parser.add_argument(
            "--kmers", help="Prints the sequence of each k-mer in the dataset (fasta).", action="store_true"
        )
        ...
        parser.add_argument(
            ...
            help="Prints the description that was assigned to the phenotype.",
            action="store_true",
        )
        parser.add_argument(
            "--phenotype-metadata",
            help="Prints the path of the file from which the phenotypic metadata" " was acquired.",
            action="store_true",
        )
        parser.add_argument(
            "--phenotype-tags", help="Prints the phenotype tags associated to the dataset", action="store_true"
        )
        parser.add_argument(
            "--splits",
        ...
        parser.add_argument(
            "--compression", help="Prints the data compression options of the dataset.", action="store_true"
        )
        parser.add_argument(
            "--classification-type", help="Prints the dataset classification type.", action="store_true"
        )
    ...
    def split(self):
        ...
        parser.add_argument(
            "--train-size",
            type=float,
            help="The proportion of the data that will be reserved for "
        ...
        parser.add_argument(
            "--train-ids",
            help="File containing the identifiers of the genomes used to train the "
        ...
        parser.add_argument(
            "--test-ids",
            help="File containing the identifiers of the genomes used to evaluate "
        ...
        parser.add_argument(
            "--random-seed",
            type=int,
            help="A random seed used for randomly splitting the data. A "
...
class KoverLearningTool(object):
    ...
    def scm(self):
        parser = argparse.ArgumentParser(
            prog="kover learn scm",
            description="Learn a conjunction/disjunction model using the Set Covering Machine algorithm.",
        )
        parser.add_argument("--dataset", help="The Kover dataset to use for learning.", required=True)
        parser.add_argument(
            "--split", help="The identifier of the split of the dataset to use for learning.", required=True
        )
        parser.add_argument(
            "--model-type",
            choices=["conjunction", "disjunction"],
            nargs="+",
        ...
        parser.add_argument(
            "--kmer-blacklist",
            help="A file containing a list of k-mers to remove from the analysis."
            "These k-mers guaranteed not to be used in the models."
            "File format: fasta file or text file with one k-mer per line.",
            required=False,
        )
        parser.add_argument(
            "--max-rules",
            type=int,
            help="The maximum number of rules that can be included in the " "model.",
            default=10,
        )
        parser.add_argument(
            "--max-equiv-rules",
            type=int,
            help="The maximum number of equivalent rules to report for "
        ...
        parser.add_argument(
            ...
            "selects the first value specified for each hyperparameter.",
            default="cv",
        )
        parser.add_argument(
            "--bound-max-genome-size",
            type=int,
            help="Specify this only if --hp-choice is bound. The "
        ...
        parser.add_argument(
            "--random-seed",
            type=int,
            help="The random seed used for any random operation. "
        ...
        parser.add_argument(
            ...
            "--n-cores",
            type=int,
            help="The number of CPUs used to select the hyperparameter values. "
        ...
        parser.add_argument(
            "--output-dir",
            help="The directory in which to store Kover's output. It will be created if " "it does not exist.",
            default=".",
        )
        parser.add_argument("-x", "--progress", help="Shows a progress bar for the execution.", action="store_true")
        parser.add_argument("-v", "--verbose", help="Sets the verbosity level.", default=False, action="store_true")
        ...
        try:
            ...
            print(
                "Error: The split (%s) does not exist in the dataset. Use 'kover dataset split' to create it."
                % args.split
            )
            exit()
        ...
        if args.hp_choice == "cv" and len(pre_dataset.get_split(args.split).folds) < 2:
            print(
                "Error: The split must contain at least 2 folds in order to perform cross-validation. "
        ...
        nb_genome_training = {
            c: (dataset.phenotype.metadata[split.train_genome_idx] == c).sum() for c in range(len(phenotype_tags))
        }
        training_groups = [
            "Group %s: %d" % (phenotype_tags[c], nb_genome_training[c]) for c in range(len(phenotype_tags))
        ]
        report += "(%s)\n" % ", ".join(training_groups)

        report += "Number of genomes used for testing: %d " % (len(split.test_genome_idx))
        nb_genome_testing = {
            c: (dataset.phenotype.metadata[split.test_genome_idx] == c).sum() if len(split.test_genome_idx) > 0 else 0
            for c in range(len(phenotype_tags))
        }
        testing_groups = [
            "Group %s: %d" % (phenotype_tags[c], nb_genome_testing[c]) for c in range(len(phenotype_tags))
        ]
        report += "(%s)\n" % ", ".join(testing_groups)
        ...
        report += ("\n%s\n" % ("AND" if model.type == "conjunction" else "OR")).join(
            [
                "%s [Importance: %.2f, %d equivalent rules]" % (str(rule), importance, len(equivalent_rules[i]))
        ...
        results = {
            "data": {"uuid": dataset.uuid, "path": dataset.path, "split": args.split},
            "cv": {
                "best_hp": {"values": best_hp, "score": best_hp_score},
                "candidate_hp": {"model_type": args.model_type, "p": args.p, "max_rules": args.max_rules},
    ...
    def tree(self):
        parser = argparse.ArgumentParser(
            prog="kover learn tree",
            description="Learn a decision tree model using the Classification And Regression Trees algorithm.",
        )
        parser.add_argument("--dataset", help="The Kover dataset to use for learning.", required=True)
        parser.add_argument(
            "--split", help="The identifier of the split of the dataset to use for learning.", required=True
        ...
        parser.add_argument(
            ...
            type=str,
            nargs="+",
            help="Hyperparameter: The criterion used to partition the leaves of the decision tree. "
            "You can specify multiple space separated values. Refer to the documentation "
            "for more information. (default: gini)",
            choices=["gini", "crossentropy"],
            default="gini",
            required=False,
        )
        parser.add_argument(
            ...
            type=int,
            nargs="+",
            help="Hyperparameter: The maximum depth of the decision tree. You can specify multiple space separated "
        ...
        parser.add_argument(
            ...
            "space separated values. Refer to the documentation for more information. (default: 2)",
            default=2,
            required=False,
        )
        parser.add_argument(
            ...
            type=str,
            nargs="+",
            help="Hyperparameter (this one is tricky so read carefully): This controls the cost of making prediction "
        ...
        parser.add_argument(
            ...
            "pruning process. Refer to the documentation for more information. (default: cv)",
            default="cv",
        )
        parser.add_argument(
            "--bound-max-genome-size",
        ...
        def parse_class_importances(class_importance_input):
            ...
            for class_name in phenotype_tags:
                try:
                    ...
                    print(
                        'Error: no class importances defined for class "{}" which is in the dataset.'.format(class_name)
                    )
                    exit()
            ...
            for i in range(len(class_importance_input)):
                if class_importance_input[i].endswith(":"):
                    try:
                        ...
                        print(
                            "Error: no class importances defined for class {} which is in the dataset.".format(
                                class_importance_input[i].replace(":", "")
                            )
                        )
                        exit()
        ...
        def confusion_matrix_to_str(confusion_matrix):
            ...
            col_width = 5
            horizontal_bar = (
                "+-" + "-" * size_header + "+" + "+".join(["-" * col_width for c in range(len(phenotype_tags))]) + "+\n"
            )
        ...
        rule_ids = {}
        id_by_node = {
            n: "{}___ex_{}___eq_{}".format(i, n.n_examples, len(equivalent_rules[n.rule]))
            if not n.is_leaf
            else "leaf___ex_{}___{}".format(
                n.n_examples,
                "__".join(
                    [
                        "{0!s}_{1:d}_{2:.8f}".format(
                            model.class_tags[c], len(n.class_examples_idx[c]), n.breiman_info.p_j_given_t[c]
                        )
                        for c in n.class_proportions
                    ]
                ),
            )
        ...
        for node_id, node in model.decision_tree:
            if not node.is_leaf:
                ...
                fasta_header = "rule_id: {}, left_child: {}, right_child: {}".format(
                    id_by_node[node], id_by_node[node.left_child], id_by_node[node.right_child]
                )
        ...
        if args.hp_choice == "cv":
            ...
            report += "Selection strategy: sample-compression bound (delta = {0:.3f}, max-genome-size = {1:d}, value = {2:.5f})\n".format(
                args.bound_delta, args.bound_max_genome_size, best_hp_score
            )
        else:
            report += "Selection strategy: No selection\n"
        ...
        report += "Class importance: {0!s}\n".format(
            ", ".join(
                ["class {0!s}: {1:.3f}".format(c, best_hp["class_importance"][i]) for i, c in enumerate(phenotype_tags)]
            )
        )
        ...
        results = {
            ...
            "cv": {
                "best_hp": {"values": best_hp, "score": best_hp_score},
                "candidate_hp": {"criterion": args.criterion, "max_depth": args.max_depth},
                "strategy": args.hp_choice,
            },
            "metrics": {"train": train_metrics, "test": test_metrics},
            "model": {
                "n_rules": len(model.decision_tree.rules),
                "depth": model.depth,
        ...
        with open(join(args.output_dir, "model.fasta"), "w") as f:
            for i, rule in enumerate(model.decision_tree.rules):
                f.write(
                    ">{0!s}, importance: {1:.2f}\n{2!s}\n\n".format(
                        rule_ids[rule]["fasta"], rule_importances[rule], rule.kmer_sequence
                    )
                )

                with open(
                    join(args.output_dir, "model_rule_{0!s}_equiv.fasta".format(rule_ids[rule]["simple"])), "w"
                ) as f_equiv:
                    f_equiv.write(
                        "\n\n".join(
                            [
                                ">rule-{0!s}-equiv-{1:d}\n{2!s}".format(
                                    rule_ids[rule]["simple"], j + 1, equiv_rule.kmer_sequence
                                )
...
class CommandLineInterface(object):
    def __init__(self):
        ...
        parser.add_argument(
            "--cite", action="store_true", help="Show the references to cite if you use Kover in your work"
        )
        parser.add_argument("--license", action="store_true", help="Show the license for your release of Kover")
        parser.add_argument("--version", action="store_true", help="Show the version of your release of Kover")
...
The most commonly used commands are:
    ...
    info       Get information about the content of a Kover dataset"""
        ...
        parser.add_argument(
            "command", help="The dataset manipulation to perform", choices=dataset_tool.available_commands
        )
    ...
    def learn(self):
        ...
        parser = argparse.ArgumentParser(
            usage="""%(prog)s learn <experiment> [<args>]
The most commonly used commands are:
    ...
    tree       Learn a decision tree model using the Classification And Regression Trees algorithm."""
        ...
        parser.add_argument(
            "command", help="The learning experiment to perform", choices=learning_tool.available_commands
        )
