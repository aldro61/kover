...
def _predictions(model, kmer_matrix, train_example_idx, test_example_idx):
    """
...
def _cross_validation(dataset_file, split_name, model_types, p_values, max_rules, n_cpu=None):
    """
    Returns the best parameter combination and its cv score
    """
    #TODO distribute on multiple CPUs
    logging.debug("There are %d hyperparameter combinations to try." % (len(model_types) * len(p_values)))


    logging.debug("Using %s CPUs." % (str(n_cpu) if n_cpu is not None else "all"))
    ...
    for hp, score in pool.imap_unordered(hp_eval_func, product(model_types, p_values)):
        if (score < best_hp_score) or \
           ...
           (score == best_hp_score and abs(1.0 - hp[1]) < abs(1.0 - best_hp["p"])):
            ...
            best_hp_score = score
            print "Found better HP:", best_hp, score
    return best_hp_score, best_hp

# def _cross_validation(dataset_file, folds, model_types, p_values, max_rules):
#     """
#     Returns the best parameter combination and its cv score
#     """
#     #TODO distribute on multiple CPUs
...
#     best_hp_score = 1.0
#     best_hp = None
#     best_hp_model_length = None
#     for hp in product(model_types, p_values):
#         model_type = hp[0]
#         p = hp[1]
#
#         fold_score_by_model_length = np.zeros((len(folds), max_rules))
...
#         score_by_model_length = np.mean(fold_score_by_model_length, axis=0)
#         best_score_idx = np.argmin(score_by_model_length)
#         hp_score = score_by_model_length[best_score_idx]
#         model_length = best_score_idx + 1
#         if (hp_score < best_hp_score) or \
#            (hp_score == best_hp_score and model_length < best_hp_model_length) or \
#            (hp_score == best_hp_score and abs(1.0 - hp[1]) < abs(1.0 - best_hp[1])):
#             best_hp = hp
#             best_hp_score = hp_score
#             best_hp_model_length = model_length
#
#     best_hp = {"model_type": best_hp[0], "p": best_hp[1], "max_rules": best_hp_model_length}
#     return best_hp_score, best_hp

# TODO: Handle callbacks
def learn(dataset_file, split_name, model_type, p, max_rules, parameter_selection, progress_callback=None,
    ...
    # Score the parameter combinations
    if parameter_selection == "bound":
        ...
        best_hp_score, best_hp = _cross_validation(dataset_file, split_name, model_type, p, max_rules)
    else:
    ...
    train_predictions, test_predictions = _predictions(predictor.model, dataset.kmer_matrix, train_example_idx, test_example_idx)
